...
# K -> CUNet -> K -> PAFT -> I -> CUNet -> I -> PIAFT -> K -> CUNet -> K -> PAFT -> I -> CUNet -> I
...
class Pipeline:
    ...
    def train_epoch(self):
        ...
        for i, batch in enumerate(tqdm(self.train_loader, desc='Train', disable=not self.verbose, dynamic_ncols=True)):
            ...
            isp_in, isp_true, ksp_in, ksp_true = batch['ispace_under'], batch['ispace'], batch['kspace_under'], batch['kspace']

            isp_pred = self.model(isp_in.to(self.device), ksp_in.to(self.device))
