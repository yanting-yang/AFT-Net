...
class Pipeline:
    def __init__(self, save_dir: Path, debug: bool = False, verbose: bool = True) -> None:
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(self.optimizer, T_0=50, T_mult=1, eta_min=1e-5)
    ...
    @staticmethod
    def _get_loader():
        train_dataset = AFTNetSliceDataset('data/v0', 'training', ['1x', '2x', '4x', '4x', '8x', '8x'], [0.], (640, 320))
        val_dataset = AFTNetVolumeDataset('data/v0', 'validation', ['4x'], [0.], (640, 320))
        test_dataset = AFTNetVolumeDataset('data/v0', 'test', ['4x'], [0.], (640, 320))

        train_loader = DataLoader(train_dataset, batch_size=2, shuffle=True, num_workers=2, drop_last=True)
        val_loader = DataLoader(val_dataset, batch_size=None, shuffle=False, num_workers=1)
        test_loader = DataLoader(test_dataset, batch_size=None, shuffle=False, num_workers=1)
    ...
    def _get_imgs(img_in: Tensor, img_pred: Tensor, img_true: Tensor, metric: dict[str, float]):
        ...
        vmax = img_true.max()
        img_in = img_in.div(vmax).clip(0, 1).mul(255).repeat(3, 1, 1).to(torch.uint8)
        img_pred = img_pred.div(vmax).clip(0, 1).mul(255).repeat(3, 1, 1).to(torch.uint8)
    ...
    def train_epoch(self):
        ...
        for i, batch in enumerate(tqdm(self.train_loader, desc='Train', disable=not self.verbose, dynamic_ncols=True)):
            ...
            isp_true, ksp_in, ksp_true = batch['ispace'], batch['kspace_under'], batch['kspace']

            ksp_pred = self.model(ksp_in.to(self.device))
            isp_pred = torchmri.fft.ifft(ksp_pred, dim=(-2, -1))
            img_pred = torchmri.utils.rss(isp_pred, dim=-3)
            img_true = torchmri.utils.rss(isp_true.to(self.device), dim=-3)

            mse_loss = F.mse_loss(
                torch.view_as_real(ksp_pred),
                torch.view_as_real(ksp_true).to(self.device),
                reduction='sum'
            ) + F.mse_loss(
            ...
            for k in losses.keys():
                ...
                self.writer.add_scalar(f'{k}/train', losses[k][-1], self.epoch * iter + i)
    ...
    def val_epoch(self):
        ...
        for i, batch in enumerate(tqdm(self.val_loader, desc='Val', disable=not self.verbose, dynamic_ncols=True)):
            ...
            batch: dict[str, Tensor]
            ksp_in, isp_true, isp_in = batch['kspace_under'], batch['ispace'], batch['ispace_under']

            ksp_pred = self.model(ksp_in.to(self.device))
            isp_pred = torchmri.fft.ifft(ksp_pred, dim=(-2, -1))

            img_in = torchmri.utils.rss(isp_in[:, :, 160:-160], dim=-3)
            img_pred = torchmri.utils.rss(isp_pred[:, :, 160:-160], dim=-3).cpu()
            img_true = torchmri.utils.rss(isp_true[:, :, 160:-160], dim=-3)

            grids = []
            for j in range(img_true.shape[0]):
                metrics.append(utils.img_metrics(img_true[j].numpy(), img_pred[j].numpy()))
                if i == 0:
                    _img_in, _img_pred, _img_true = self._get_imgs(img_in[j], img_pred[j], img_true[j], metrics[-1])
    ...
    def test_epoch(self):
        ...
        for i, batch in enumerate(tqdm(self.test_loader, desc='Test', disable=not self.verbose, dynamic_ncols=True)):
            ...
            batch: dict[str, Tensor]
            ksp_in, isp_true = batch['kspace_under'], batch['ispace']

            ksp_pred = self.model(ksp_in.to(self.device))
            isp_pred = torchmri.fft.ifft(ksp_pred, dim=(-2, -1))

            img_true = torchmri.utils.rss(isp_true[:, :, 160:-160], dim=-3).numpy()
            img_pred = torchmri.utils.rss(isp_pred[:, :, 160:-160], dim=-3).cpu().numpy()
            for j in range(img_true.shape[0]):
